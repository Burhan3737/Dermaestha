...
import { AdminDoctors } from '#src/modules/admin/views/AdminDoctors/AdminDoctors.jsx';
...
describe('AdminDoctors (A-01)', () => {
  ...
  it('reset-password modal posts the admin-set password (DA5)', async () => {
    ...
    await waitFor(() =>
      expect(api.post).toHaveBeenCalledWith('/doctors/d1/reset-password', {
  ...
  it('cancelling the reset modal clears the typed password and stale error', async () => {
    api.post.mockRejectedValue(
      Object.assign(new Error('Too weak'), { code: 'VALIDATION', status: 400 }),
    );
    renderView();
    await screen.findByText('Dr Ayesha Khan');
  ...
  it('photo upload failure after create keeps the form open and surfaces the error', async () => {
    ...
    api.upload.mockRejectedValue(
      Object.assign(new Error('Photo too large'), { code: 'INVALID_FILE', status: 400 }),
    );
    renderView();
    await screen.findByText('Dr Ayesha Khan');
