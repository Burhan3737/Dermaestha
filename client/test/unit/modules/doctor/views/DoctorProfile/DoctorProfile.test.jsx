...
import { DoctorProfile } from '#src/modules/doctor/views/DoctorProfile/DoctorProfile.jsx';
import { api } from '#src/lib/apiClient/apiClient.js';

vi.mock('#src/lib/apiClient/apiClient.js', () => ({ api: { get: vi.fn(), post: vi.fn() } }));
vi.mock('#src/context/session/session.jsx', () => ({ useSession: () => ({ session: null }) }));
...
describe('P-03 Doctor profile', () => {
  ...
  it('exposes day navigation and fetches a future day’s slots (ISSUE-1)', async () => {
    ...
    api.get.mockImplementation((path) => {
      ...
      return Promise.resolve({
        data: [{ slotStart: `${date}T09:00:00.000Z`, slotEnd: `${date}T09:30:00.000Z` }],
      });
    });
