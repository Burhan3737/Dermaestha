...
describe('appointment.getForRole', () => {
  it('returns a confirmed appointment detail with a refundQuote for the owner', async () => {
    prisma.appointment.findUnique.mockResolvedValue({ id: 'a1', patientUserId: 'u1', state: 'confirmed', slotStart: new Date('2099-01-04T13:00:00Z'), slotEnd: new Date('2099-01-04T13:30:00Z'), feeAtBooking: 250000, forSelf: true, subjectName: null, doctorId: 'd1', doctor: { id: 'd1', specialization: 'Acne', photoUrl: null, user: { fullName: 'Dr A' } } });
  ...
  it('hides another patient\'s appointment as 404', async () => {
    prisma.appointment.findUnique.mockResolvedValue({ id: 'a1', patientUserId: 'other', state: 'confirmed', doctor: { id: 'd1', specialization: 'Acne', photoUrl: null, user: { fullName: 'Dr A' } } });
