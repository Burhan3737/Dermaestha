...
import { payfastMock, buildSignedIpn, signParams } from '#src/integrations/payment/payfast.mock.js';
...
describe('payfast.mock gateway', () => {
  ...
  it('verifyWebhook throws 401 on a bad signature', () => {
    ...
    expect(() =>
      payfastMock.verifyWebhook({ body: { ...ipn, signature: 'tampered' } }),
  ...
  it('refund returns a settled refundRef keyed by idempotencyKey', async () => {
    const out = await payfastMock.refund({
      providerRef: 'mock_1',
      amount: 240000,
