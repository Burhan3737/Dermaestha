...
vi.mock('#src/modules/notification/service.js', () => ({ enqueue: vi.fn().mockResolvedValue({}) }));
...
import { recordManualRefund } from '#src/modules/admin/service.js';
...
describe('admin.listRecords (F13.01)', () => {
  ...
  it('composes the filter superset into the where clause', async () => {
    ...
    expect(arg.where.payments.some.OR).toEqual([{ providerRef: 'pf_ok' }, { refundRef: 'pf_ok' }]);
