...
import { list, create, update } from '#src/modules/medicine/service.js';
import { list as listController } from '#src/modules/medicine/controller.js';
...
describe('medicine.create / update (F11.02/.03)', () => {
  ...
  it('update of an unknown id (P2025) maps to 404 NOT_FOUND', async () => {
    prisma.medicine.update.mockRejectedValue(
      Object.assign(new Error('not found'), { code: 'P2025' }),
    );
    await expect(
      update({ id: 'nope', data: { unitPrice: 1 }, actorId: 'a' }),
  ...
  it('a non-P2025 failure propagates instead of masquerading as 404', async () => {
    ...
    await expect(update({ id: 'm1', data: { unitPrice: 1 }, actorId: 'a' })).rejects.toThrow(
      'db down',
    );
  });
});
