...
describe('prescription flow — immutable submit, corrections, snapshot durability', () => {
  ...
  beforeAll(async () => {
    ...
    // A confirmed past consultation for a third party (P8 who-for).
    appointmentId = (
  ...
  it('doctor submits: prescription + snapshots + state + outbox job, all committed', async () => {
    ...
    expect(appt.state).toBe('confirmed'); // prescriptions do not change appointment state
...
describe('prescription flow — concurrency race guard + forSelf snapshot', () => {
  ...
  beforeAll(async () => {
    ...
    // Dedicated appointment for the Postgres-level race test — confirmed → cancelled.
    raceApptId = (
      await prisma.appointment.create({
        data: {
          doctorId,
  ...
  it('the state-guarded transition write loses cleanly when racing (real Postgres row lock)', async () => {
    // T1: open a transaction, move confirmed → cancelled, then HOLD the row lock.
    ...
    const t2 = appointmentService
      .transition({ appointmentId: raceApptId, to: 'cancelled', actorType: 'system' })
      .then(
        () => 'won',
        (e) => e.code,
      );
    ...
    const audits = await prisma.auditLog.count({
      where: { eventType: 'appointment.cancelled', targetRef: raceApptId },
    });
