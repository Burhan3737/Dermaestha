...
6. **Remaining doc cleanup (pre-existing, NOT fixed — flag):** doc 13 (and doc 06) reuse some screen IDs for two screens each (P-13 = prescription-view AND profile/account; D-05 = builder AND history). The genuinely-unbuilt M4 view items (272–273) stay `[ ]` correctly, but the embedded labels are imprecise. Best resolved in a dedicated doc-06↔doc-13 screen-ID reconciliation, not piecemeal.
