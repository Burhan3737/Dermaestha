...
**Status:** Complete — Milestone 0 foundation done (Tasks 0–13)
...
| `README.md` | Created | Developer runbook: overview, prerequisites, step-by-step local setup (install, .env, Postgres, prisma generate, migrate, seed, admin bootstrap), dev/prod/Docker run modes, test commands, migration caveat (uniq_active_slot partial index), admin bootstrap notes, M0 project structure. |
...
- Final code review (whole branch, Opus model): **zero Critical**, verdict = merge. Review-driven fixes applied (commit `faad2ba`): `app.set('trust proxy', 1)` in `index.js` (prod Secure cookies + correct rate-limit IP keying behind a TLS proxy), client dev `/api`→`:3000` proxy in `client/vite.config.js`, and a schema-header comment accuracy tidy. Server suite still **20/20** after the fixes.

## Open items / next session
- Milestone 0 complete. All 14 tasks (0–13) done.
...
- Task 13 — README/runbook created at repo root: covers Overview, Prerequisites, step-by-step local setup (npm install → prisma generate → migrate → seed → admin bootstrap), Dev/Prod/Docker run modes, testing instructions, migration caveat (uniq_active_slot partial index must be re-appended if init migration is recreated), and admin bootstrap notes. Milestone 0 complete.
...
- Task 13 — `npx vitest run` (server suite): **8 files, 20 tests passed** (Vitest v2.1.9, duration 1.38s). All 8 test files green: errorHandler.test.js (3), integrations.test.js (3), env.test.js (3), requireRole.test.js (3), audit.service.test.js (2), doubleBooking.test.js (1), password.test.js (2), app.integration.test.js (3).
- Task 13 — `npm --workspace client run test`: **1 file, 2 tests passed** (Vitest v2.1.9, jsdom, duration 2.23s). RoleRoute: children when role matches + redirects when mismatched.
