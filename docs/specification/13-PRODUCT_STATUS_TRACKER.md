...
| **M1 — Booking flow**          | End of week 2 | In progress | ~30%                    | Slice A (auth) merged: auth service/controller/routes, DA3 gate, lockout, reset-token storage + P-04/P-05/D-01 and the client auth foundation (Context + TanStack Query). Booking/doctor/availability/slot services and remaining views pending (Slices B–C) |
...
| Shared Zod DTO seam             | In progress | `shared/schemas/auth.js` + `index.js` — auth DTOs added (Slice A); booking/payment DTOs pending                                                                                      |
...
| Route config seam (`routes.jsx`)                              | In progress | `client/src/routes.jsx` — auth routes wired (signup/login/forgot/reset/change) + role placeholders; patient/doctor/admin views + RoleRoute guards pending |
| Client-side role guard (`RoleRoute.jsx`)                      | Done        | `client/src/lib/RoleRoute.jsx` — tested (2/2)                                                                                |
| 24 views (P-01…P-13, D-01…D-06, A-01…A-05)                    | In progress | P-04, P-05 (+ forgot/reset), D-01 built                                                                                      | 3 of 24 built (Slice A auth screens); rest pending   |
| Layouts (`TopNavLayout`, `BottomTabsLayout`, `SidebarLayout`) | In progress | `AuthSplitLayout` built                                                                                                      | AuthSplit done (Slice A); 3 chrome layouts pending (Slice B) |
| Shared design components (~16)                                | In progress | `Button, Field, Card, Alert, Checkbox`                                                                                       | 5 of ~16 built (Slice A); rest as views need them    |
