...
| **M1 — Booking flow**          | End of week 2 | In progress | ~55%                    | Slice A (auth) + Slice B (discovery & availability) merged: auth + doctor + availability services/routes, DA3 gate, lockout, reset-token storage, 30-min slot generation (date-fns-tz/ADR-21) + `BLOCK_HAS_BOOKINGS` guard, and client P-02/P-03/D-03/P-04/P-05/D-01 + 3 nav layouts (Context + TanStack Query). Slot-lock/booking + payment services and P-06 booking view pending (Slice C) |
...
| Shared Zod DTO seam             | In progress | `shared/schemas/auth.js` + `availability.js` + `index.js` — auth DTOs (Slice A) + availability/doctor-list/slots DTOs (Slice B); booking/payment DTOs pending                                                                                      |
...
| Route config seam (`routes.jsx`)                              | In progress | `client/src/routes.jsx` + `App.jsx` — auth routes + discovery routes wired (`/` listing, `/doctors/:id` profile, `/doctor/availability` RoleRoute-guarded); booking/admin views pending |
| Client-side role guard (`RoleRoute.jsx`)                      | Done        | `client/src/lib/RoleRoute.jsx` — tested (2/2)                                                                                |
| 24 views (P-01…P-13, D-01…D-06, A-01…A-05)                    | In progress | P-04, P-05 (+ forgot/reset), D-01 (Slice A); P-02, P-03, D-03 (Slice B)                                                      | 6 of 24 built; rest pending                          |
...
| Shared design components (~16)                                | In progress | `Button, Field, Card, Alert, Checkbox` (Slice A); `DoctorCard, SlotButton` (Slice B)                                         | 7 of ~16 built; rest as views need them              |
...
- [~] Patient views (client): P-02 doctor listing ✓, P-03 doctor profile ✓, P-04 sign-up ✓, P-05 login + recovery ✓ (Slice A/B); P-06 booking (slot + who-for) pending (Slice C)
