...
| **M3 — Prescriptions**         | End of week 6 | In progress | ~85–90%               | Slice F built the F08 prescription backend (immutable submit + snapshots + `completed→prescription_issued`), the F11 medicine backend (search + admin create/update), the `prescription_ready` outbox trigger, and the patient/doctor UI (P-09/P-13/D-05 + D-02 additions) incl. the client PDF renderer. Remaining: final `prescription_ready` template copy (M4) and the A-02 admin medicine UI (Slice G/M4) |
...
| 11  | Medicine catalogue          | M3        | Built (backend) | `server/src/modules/medicine/service.js`, `server/src/modules/medicine/controller.js`, `server/src/modules/medicine/index.js`                  | Slice F: `GET /api/medicines?search=` (active-only, name-sorted, search on name+genericName) + admin `POST`/`PATCH /api/admin/medicines` (deactivate-only via `isActive`; audit `medicine.created`/`medicine.updated`). A-02 admin catalogue UI still Slice G/M4. Tested. |
...
| Shared Zod DTO seam             | In progress | `shared/schemas/auth` + `doctor` + `appointment` + `medicine` + `prescription` + `index.js` — auth (Slice A) + availability/doctor-list/slots (Slice B) + lock/cancel (Slice C) + medicine search/create/update + prescription submit DTOs (Slice F); remaining admin DTOs pending                                               |
...
| 24 views (P-01…P-13, D-01…D-06, A-01…A-05)                    | In progress | P-04, P-05 (+ forgot/reset), D-01 (Slice A); P-02, P-03, D-03 (Slice B); P-06, P-07, P-08, P-10 (Slice C); P-09 past appointments, P-13 prescription view, D-05 prescription builder + D-02 write-prescription action/awaiting badge (Slice F) | 13 of 24 built; rest pending                         |
...
| F11 — Admin: medicine catalogue                      | Medicine (11)                               | M3        | Built (backend) | Slice F: `GET /api/medicines?search=` (active-only, name+genericName) + admin `POST`/`PATCH /api/admin/medicines` (deactivate-only via `isActive`; audit rows); never alters existing prescription snapshots (#5). A-02 admin catalogue UI still Slice G/M4. Tested |
...
- [x] Medicine service: admin create/update, unit price, deactivate (via `isActive` toggle; no DELETE) — Slice F (backend; A-02 admin UI is Slice G/M4)
