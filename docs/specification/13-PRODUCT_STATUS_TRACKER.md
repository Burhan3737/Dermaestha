...
**Last verified:** 2026-06-14 (post-Slice H · S3). Statuses are grounded in repo inspection; anything not verifiable in code is marked `Not verified`. Stub files that only throw `AppError('NOT_IMPLEMENTED', ..., 501)` are counted as **In progress** (seam exists, real logic absent).
...
| **M4 — Launch-ready**          | End of week 8 | In progress | ~65%                  | Slice G built F10–F14 backends + A-01..A-05 views + DA5 reset; Slice H · S1 wired the PayFast Pakistan adapter, S2 the Daily.co adapter (ADR-33), and S3 the video consultation UI (P-11/P-12/D-04 + the `track.js` KPI seam, ADR-34); remaining: landing page, legal content, email template copy, full E2E QA                       |
...
| 18  | Analytics/telemetry         | M2→M4     | In progress | `client/src/lib/analytics/track.js`                                                                                                            | `AnalyticsEvent` model in schema. Slice H · S3 added the client-side KPI #3 emit seam `track.js` — fire-and-forget `track(type, meta)` POSTing `{ type, networkType, meta }` to `/api/analytics/events`, swallowing all errors (`.catch(() => {})`) so it no-ops until S6 ships the route. Emits `video_join_attempt` (Join click, P-08 + D-02) and `video_join_success` (Daily `joined-meeting`, P-12/D-04). Server analytics service + the `POST /api/analytics/events` route + the KPI #1 landing emit remain S6/S4. |
...
- [~] Patient views (client): P-11 pre-call get-ready room ✓, P-12 video consultation ✓ (Slice H · S3); patient Profile tab (doc 06 §2 nav destination; no dedicated screen-inventory ID) still pending. Refund status shows within the P-08/P-09 dashboards (doc 02 §F04), not a standalone screen
- [ ] Full E2E QA pass
