...
| **M2 — Video + Payments**      | End of week 4 | In progress | ~75%                    | Slice C built Payments (intent + signed-webhook atomic commit, `feeAtBooking`), Refund (net-of-fee, idempotent), Cancellation (patient/doctor), and the Appointment state-machine writer — exercised via a dev mock PayFast gateway (ADR-22). Slice D built Video (token issuance + window guard, daily.mock dev simulator, real webhook join-recording, ADR-24) and the appointment-evaluation worker (`node-cron`, `evaluation.service`, ADR-25). Still absent: reminder cadence + reconciliation workers, concrete PayFast/Daily.co network adapters |
...
| 9   | Video                       | M2        | Built       | `server/src/integrations/video/daily.stub.js`, `server/src/integrations/video/daily.mock.js`, `server/src/integrations/video/index.js`, `server/src/services/video.service.js`, `server/src/routes/appointments.js` (`GET /:id/video-token`), `server/src/routes/webhooks.js` (`POST /daily`), `server/src/workers/` | Slice D: video-token issuance with slot-window guard (`VIDEO_WINDOW_CLOSED`); real `POST /api/webhooks/daily` handler recording `doctorJoinedAt`/`patientJoinedAt`; dev mock (`daily.mock`) + `/dev/video/*` simulator + `/dev/worker/*` on-demand trigger (ADR-24); appointment-evaluation `node-cron` worker + `evaluation.service.js` (ADR-25). Concrete Daily.co network adapter (`daily.js`) pending (P-12).                                                                                   |
...
| Appointment-evaluation (state advancement) | M2        | Built       | `server/src/workers/` (`node-cron`), `server/src/services/evaluation.service.js` | Slice D: `evaluateDueAppointments(now)` — activation, no-show resolution (ADR-12), completion; driven by `* * * * *` cron; `/dev/worker/evaluate` on-demand trigger (ADR-25). |
