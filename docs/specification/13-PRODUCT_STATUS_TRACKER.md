...
**Last verified:** 2026-06-14 (post-Slice H · S4). Statuses are grounded in repo inspection; anything not verifiable in code is marked `Not verified`. Stub files that only throw `AppError('NOT_IMPLEMENTED', ..., 501)` are counted as **In progress** (seam exists, real logic absent).
...
| **M4 — Launch-ready**          | End of week 8 | In progress | ~75%                  | Slice G built F10–F14 backends + A-01..A-05 views + DA5 reset; Slice H · S1 wired the PayFast Pakistan adapter, S2 the Daily.co adapter (ADR-33), S3 the video consultation UI (P-11/P-12/D-04 + the `track.js` KPI seam, ADR-34), and S4 the public surface — P-01 landing (`/`) + F16 legal pages (`/legal/terms`,`/legal/privacy`) as a banner-marked structured DRAFT + the KPI #1 `landing_view`/`booking_started` client emits (ADR-35); remaining: email template copy, F16 final lawyer-reviewed legal copy (pre-launch gate), full E2E QA                       |
...
| 18  | Analytics/telemetry         | M2→M4     | In progress | `client/src/lib/analytics/track.js`                                                                                                            | `AnalyticsEvent` model in schema. Slice H · S3 added the client-side KPI #3 emit seam `track.js` — fire-and-forget `track(type, meta)` POSTing `{ type, networkType, meta }` to `/api/analytics/events`, swallowing all errors (`.catch(() => {})`) so it no-ops until S6 ships the route. Emits `video_join_attempt` (Join click, P-08 + D-02) and `video_join_success` (Daily `joined-meeting`, P-12/D-04). Slice H · S4 added the KPI #1 funnel emits over the same seam — `landing_view` (P-01 mount, `{ referrer? }`) + `booking_started` (slot-lock success in `useBooking.js`, `{ doctorId }`). Server analytics service + the `POST /api/analytics/events` route + the server-side terminal `booking_confirmed` emit remain S6. |
...
- [~] Analytics: client KPI #3 emit seam built — `client/src/lib/analytics/track.js` fires `video_join_attempt` (Join click) + `video_join_success` (Daily `joined-meeting`), fire-and-forget to `POST /api/analytics/events` (Slice H · S3, ADR-34). Server analytics service/route + KPI #1 (landing→booking) emit pending (S6/S4)
