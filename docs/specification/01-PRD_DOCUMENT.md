...
3. **Deliver reliable paid booking** — ensure slot double-booking rate is zero (enforced at the storage layer) and payment success rate across all four payment methods (cards, JazzCash, Easypaisa, bank transfer) reaches ≥90%.
...
- **Book & pay for a slot** — slot selection, 10-minute lock during payment flow, redirect to payment aggregator hosted checkout, confirmation on successful webhook
...
- **Cancel & refund** — in-app cancellation with automatic refund (net of gateway fee) for cancellations ≥2 hours before slot start; no refund inside the 2-hour window
...
- **View upcoming appointments** — dashboard section listing all confirmed and in-progress appointments with Join Call and Cancel affordances
...
- **Admin system-health alerts** — alert feed for payment webhook mismatches, refund failures, email failures, overdue prescriptions, and unhandled exceptions
- **Admin records & audit log** — unified searchable view of appointments, payments, and the full append-only audit trail; read-only with admin-only mutations (email re-trigger, disputed flag)
- **Admin platform settings** — configurable minimum booking lead time (default 1 hour, down to 30 minutes) and fallback transaction-fee model
...
- **Append-only audit trail** — every state transition, auth event, payment event, refund event, and admin action flows through the same append-only audit log; no update or delete path exposed
...
- **Transactional email** — six trigger types (booking confirmation, 24-hour reminder, 1-hour reminder, prescription-ready notification, refund confirmation, cancellation apology) with exponential-backoff retry and admin alert on final failure
- **Pakistan-compatible payment aggregator** — accepts cards, JazzCash, Easypaisa, and bank transfer through a hosted checkout with signed webhooks, refund API, and reconciliation query
...
1. **Payment-aggregator merchant KYC delay** (Likelihood: High, Impact: High) — blocks the M2 milestone; mitigated by starting the merchant application in Week 1 in parallel with development.
