...
- The Settings page also exposes a **fallback transaction-fee model** used only when the payment aggregator does not report a per-transaction fee for a payment (per §4.1 #5): a configurable percentage and/or fixed PKR amount, with a documented default and validated bounds. This figure feeds the refund amount, the cancellation-modal estimate, and the dashboard refund breakdown (P6) identically. When the aggregator *does* report a fee, that reported figure always wins. Each change is recorded in the audit log as an admin-actor entry (§3.6)
...
2. **Reconciliation safety net.** Hourly worker → aggregator reconciliation query → for each unconfirmed payment with a successful aggregator status, app core completes the same atomic commit as the webhook path → admin alert on mismatch. If the slot's lock had expired and the slot was already confirmed to another patient, the commit fails on storage uniqueness (§3.3 #1) and **no second appointment is created**; the paying patient is auto-refunded in full (platform-caused, not a cancellation), alerted to the admin, and notified by email (§4.4 #6a).
...
- **Abuse protection on public endpoints.** Login (DA2), forgot-password (P2), sign-up (P2), and payment-intent creation (§3.4) are rate-limited per source identity and per target account. Repeated failed logins against one account trigger a temporary backoff/lockout (threshold and duration are an architecture decision, but must be specified and audit-logged per §3.6). Payment-intent creation is rate-limited per patient to protect the aggregator's API quota (beyond the `(patient, slot)` idempotency of §3.3 #7). Threshold breaches are recorded in the audit log and, for sustained abuse, surfaced to the admin alert feed (A3).
...
| 7 | **No-show grace** | 15 minutes after slot start. Patient absent at slot+15 → `patient_no_show` (no refund). Doctor absent at slot+15 → `doctor_no_show` (full refund minus gateway fee + apology email). **If neither party joins by slot+15, doctor-absence takes precedence → `doctor_no_show` (refund net of gateway fee + apology email); the patient is never penalized for an appointment the doctor also missed (§4.3).** | Industry standard in telemedicine. |
...
- `in_progress → completed`: finalized at **slot-end + 5 min** once both parties have joined at least once. A transient mid-call disconnect does **not** finalize completion — either party may rejoin the same room until slot-end + 5 min (edge #22). An earlier finalization on "both parties left" is permitted only when the departure is an explicit end-of-call action by both parties, not a dropped connection; absent that explicit signal, completion waits for the slot-end + 5 min cutoff. Join/leave facts are sourced from the video provider's participant events (§3.4). The doctor never manually marks completion in v1.
- `in_progress → patient_no_show` / `doctor_no_show`: at slot-start + 15 min, the appointment-evaluation job inspects recorded join events. **If the doctor never joined (whether or not the patient joined) → `doctor_no_show` (refund initiated net of gateway fee + apology email).** If the doctor joined but the patient never joined → `patient_no_show` (no refund). If both joined, the no-show path is not taken. _Rationale: doctor absence takes precedence because the platform failed to deliver the contracted doctor; the patient is not penalized for an appointment the doctor also missed._
- **Missing participant data at evaluation.** If the video provider's participant events are unavailable or unreadable when the appointment-evaluation job runs at slot-end + 5 min, the job must still move the appointment out of `in_progress`: it resolves to a non-penalizing terminal state (treated as `doctor_no_show` for refund purposes so the patient is not charged for an unverifiable consultation) **and** raises an admin alert (A3) flagging that the resolution was made without join data, so the admin can adjudicate via A5. The appointment must never remain `in_progress` past slot-end + 5 min.
...
- The order screen lists the prescription's catalogue medicines, each with an **editable quantity**. v1 of this module does **not** parse free-text dosage/duration into a quantity; the quantity defaults to a safe value (1, or blank requiring entry) and is always patient-editable. The unit price (snapshotted per §6.4) and a computed subtotal + delivery fee + total update as quantities change. Admin confirms stock and final pricing before charge/dispatch (MO2); for COD this confirmation precedes dispatch, for card it precedes capture
